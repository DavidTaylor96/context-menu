...
const HoldItemComponent = ({
  ...
  //#region functions
  const hapticResponse = hapticFeedback => {
    // Define the style of haptic feedback, default to 'Medium' if not provided
    const style = hapticFeedback || 'Medium';

    // Define vibration patterns for different haptic feedback styles
    const patterns: any = {
      Selection: [0, 10, 20, 30], // Example pattern for selection
      Light: [0, 15], // Example pattern for light impact
      Medium: [0, 30], // Example pattern for medium impact
      Heavy: [0, 45], // Example pattern for heavy impact
      Success: [0, 30, 60, 90], // Example pattern for success notification
      Warning: [0, 45, 90, 135], // Example pattern for warning notification
      Error: [0, 60, 120, 180], // Example pattern for error notification
    };

    // Use the Vibration API to trigger the haptic feedback
  ...
  const onCompletion = (isFinised?: boolean) => {
    ...
    if (isFinised && isListValid) {
      ...
      if (hapticFeedback !== 'None') {
        runOnJS(hapticResponse)(hapticFeedback);
      }
    }
